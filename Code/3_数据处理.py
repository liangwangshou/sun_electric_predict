...
train_data=pd.read_csv(file_path+'train/train_{}.csv'.format(num),parse_dates=['时间'],encoding='utf-8')
...
test_data=pd.read_csv(file_path+'test/test_{}.csv'.format(num),parse_dates=['时间'],encoding='utf-8')
...
# data.to_csv('hhh.csv')
# In[19]:
data['month']=data['年月日'].apply(lambda x:int(str(x).split('-')[1]))
# print(data.columns)
...
data.to_csv(file_path+'final_data'+str(num)+'.csv',encoding='gbk',index=False)
