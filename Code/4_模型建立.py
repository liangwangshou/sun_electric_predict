...
import pandas as pd
data=pd.read_csv(file_path+'final_data'+str(num)+'.csv',encoding='gbk',parse_dates=['时间','ds'])
...
import numpy as np
contin_col=[x for x in contin_col if x in data.columns]
...
# 数据拆分
x_train, x_valid, y_train, y_valid = train_test_split(train_x, train_y, test_size=0.25, random_state=2020)
# 提前停止回调
early_stopping = EarlyStopping(monitor='val_loss', patience=10, verbose=0)
# 训练模型
history = model_dnn.fit(x_train, y_train, validation_data=(x_valid, y_valid), callbacks=[early_stopping])

model_dnn.model.save('saved_model.h5')
# from tensorflow.keras.models import load_model
# # 加载模型
# model_dnn = load_model('saved_model.h5')
...
predict.to_csv(file_path+'predict/predict_'+str(num)+'.csv',encoding='gbk',index=False)
